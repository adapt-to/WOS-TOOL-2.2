...
class slot_con(QtGui.QWidget, Ui_Form): # 定义槽函数
	...
	def on_pushButton_1_clicked(self):
		...
		fff = self.lineEdit_1.text()
		if fff:
			browser = webdriver.Chrome(r"C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe")
			browser.get('https://apps.webofknowledge.com/')

			input = browser.find_element_by_id("value(input1)")
			input.send_keys(str(fff))
			input.send_keys(Keys.ENTER)
			wait = WebDriverWait(browser, 10)
			self.article.target = browser.current_url
			browser.close()
		else:
			self.con_serial("检索条件为空，请重新输入！！！")
	...
	def on_pushButton_7_clicked(self):
		...
		if fff:
			...
			wait = WebDriverWait(browser, 10)
			#self.article.target = browser.current_url
			#browser.get(self.article.target)
			action = ActionChains(browser)
			action.click(browser.find_element_by_id("TC.D;PY.D;AU.A;SO.A;VL.D;PG.A")).perform()
			wait = WebDriverWait(browser, 10)
			self.article.target = browser.current_url
			browser.close()
		else:
	...
	def on_pushButton_5_clicked(self):
		...
		if fff:
			...
			# browser.get(self.article.target)
			action = ActionChains(browser)
			action.click(browser.find_element_by_id("ACA.D;PY.D;LD.D;SO.A;VL.D;PG.A;AU.A")).perform()
			wait = WebDriverWait(browser, 10)
			self.article.target = browser.current_url
			browser.close()
		else:
	...
	def on_pushButton_6_clicked(self):
		...
		if fff:
			...
			# browser.get(self.article.target)
			action = ActionChains(browser)
			action.click(browser.find_element_by_id("RS.D;PY.D;AU.A;SO.A;VL.D;PG.A")).perform()
			wait = WebDriverWait(browser, 10)
			self.article.target = browser.current_url
			browser.close()
		else:
	...
	def on_pushButton_2_clicked(self):
		...
		if not self.article.path:
			#self.article.dir = "D:\\WOS文献爬取" +"\\"
			self.article.path = "D:\\"+ "WOS文献" +time.strftime("-%Y-%m-%d %H %M %S", time.localtime()) + ".txt"
...
class DownArticle():
	...
	def get_contents(self,target):
		...
		search_souretitle = bf.find_all("p", class_="sourceTitle") # 查找期刊

		Factor = []
		search_fasctor = bf.find_all("table", class_="Impact_Factor_table")
		if len(search_fasctor) != 0:
			a = search_fasctor[0].find_all("td")
			for i in a:
				Factor.append(i.text)
		if len(Factor) != 0:
			factor = "/".join(Factor)
		else:
			factor = "未检索到影响因子数据"
...
class slot(QtGui.QMainWindow):
	...
	def graphical_intf(self):
		...
		self.con.setGeometry(QtCore.QRect(0, 0, 484, 634))
	# self.con.show()
...
if __name__ == "__main__":
	...
	widget.resize(484, 634)
	a = QtGui.QFrame(widget)
	a.setGeometry(QtCore.QRect(0, 0, 484, 634))
	w = slot(widget,a)
	w.graphical_intf()
